...
- Chuẩn hóa test cases sang Bảng (Table) (Jira/Excel format)
...
5. **Xuất ra format Bảng (Table)** chuẩn, sẵn sàng convert sang Excel/Jira
...
|---|---|---|---|---|---|---|---|---|
| PPAC_LOGIN_TC_001 | Login | High | Đăng nhập với thông tin hợp lệ | Đã có tài khoản active | 1. Mở trang Login<br>2. Nhập email + password hợp lệ<br>3. Nhấn Đăng nhập | auto_login_20260602_A3F2@test.com | Điều hướng tới Dashboard, hiển thị tên user | High |
```

Quy tắc:
- Cột đầu tiên PHẢI là `TC ID` (ID viết HOA, chỉ chữ/số/`_`, vd `PPAC_LOGIN_TC_001`).
...
- Tương thích script `scripts/export_testcases_to_excel.ps1` (table parser nhận diện bảng qua cột `TC ID`).
...
**Mục đích:** Đóng gói test cases thành format Bảng (Table) chuẩn, sẵn sàng convert sang Excel/Jira.

**Agent phải:**
1. Chuẩn hóa toàn bộ test cases sang **Bảng (Table)** — mỗi TC là một dòng, cột đầu tên `TC ID`:

```markdown
| TC ID | Module | Risk Level | Test Scenario | Pre-Condition | Test Steps | Test Data | Expected Result | Priority |
|---|---|---|---|---|---|---|---|---|
| CRM_CUST_TC_001 | Customer Management | High | Tạo khách hàng mới với dữ liệu hợp lệ | Đã đăng nhập với quyền Sales | 1. Mở màn hình Create Customer<br>2. Nhập tên, email, số điện thoại hợp lệ<br>3. Nhấn Save | auto_createCustomer_20260602_A3F2@test.com | Hiển thị "Tạo thành công"; KH xuất hiện trong danh sách | High |
```

2. Quy tắc Bảng (Table):
   - Cột đầu tiên PHẢI là `TC ID` — ID viết HOA, chỉ chữ/số/`_` (ví dụ: `CRM_CUST_TC_001`).
   - Thứ tự cột chuẩn: `TC ID`, `Module`, `Risk Level`, `Test Scenario`, `Pre-Condition`, `Test Steps`, `Test Data`, `Expected Result`, `Priority`.
   - Field nhiều dòng (Test Steps, Expected Result...): các bước/ý xuống dòng trong ô bằng `<br>`, đánh số.
   - Mỗi TC một dòng — KHÔNG gộp nhiều TC vào một dòng.
   - **TUYỆT ĐỐI không được bỏ sót** bất kỳ test case nào đã sinh ở Bước 5.
   - Nếu quá dài → tách thành nhiều bảng con theo màn/nhóm (Part 1, Part 2...) và hỏi user để tiếp tục.
3. Xuất output dưới dạng Artifact (`test_cases_<module>.md`).

**Output:** Bộ Test Cases dạng Bảng hoàn chỉnh (tương thích `scripts/export_testcases_to_excel.ps1`).
...
- ❌ Sinh test data chung chung / placeholder
- ❌ Rút gọn hoặc bỏ sót test case khi mapping sang Bảng (Table)
...
| Output | Mô tả |
|--------|--------|
| TC dạng Bảng | Test Cases đầy đủ (bảng markdown, cột đầu `TC ID`), sẵn sàng convert sang Excel/Jira |
...
| 6 | Bảng (Table) chuẩn (Jira/Excel ready) |
