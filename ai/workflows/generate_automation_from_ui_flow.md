...
> **BẮT BUỘC (MANDATORY SKILL):** Bạn PHẢI nạp và đọc kỹ nội dung của skill **`ui_debug_agent`** (nạp qua công cụ Skill — tự resolve theo tên) trước khi bắt đầu. Ngoài ra tham khảo thêm skill **`smart_locator_agent`** để sinh locator ổn định và **`qa_automation_engineer`** cho quy tắc automation chung.
...
4. **Locator Priority** (tuân thủ `ai/rules/locator_strategy.md`):
