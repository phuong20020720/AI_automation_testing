...
> **BẮT BUỘC (MANDATORY SKILL):** Bạn PHẢI nạp và đọc kỹ nội dung của skill **`rbt_manual_testing`** (nạp qua công cụ Skill — tự resolve theo tên) trước khi bắt đầu thực hiện tác vụ này. Sử dụng **Mode FULL RBT** của skill. Ngoài ra, tham khảo thêm skill **`requirements_analyzer`** để hiểu cách phân tích giao diện nếu cần.
...
1. Đóng gói toàn bộ test cases sang **Bảng (Table)** — mỗi TC là một dòng, cột đầu tên `TC ID`:
---
   | TC ID | Module | Risk Level | Test Scenario | Pre-Condition | Test Steps | Test Data | Expected Result | Priority |
   |---|---|---|---|---|---|---|---|---|
   | CRM_CUST_TC_001 | Customer Management | High | Tạo khách hàng mới với dữ liệu hợp lệ | Đã đăng nhập với quyền Sales | 1. Mở màn hình Create Customer<br>2. Nhập dữ liệu hợp lệ, nhấn Save | auto_createCustomer_20260602_A3F2@test.com | Hiển thị "Tạo thành công" | High |
---
   (cột đầu PHẢI là `TC ID`; field nhiều dòng xuống dòng trong ô bằng `<br>`, đánh số; mỗi TC một dòng)
2. Không được bỏ sót test case nào
3. Xuất dưới dạng Artifact nếu dài

## Output

- Bộ Test Cases dạng Bảng hoàn chỉnh, sẵn sàng convert sang Excel/Jira/TestRail (`scripts/export_testcases_to_excel.ps1`)
- Traceability Matrix
