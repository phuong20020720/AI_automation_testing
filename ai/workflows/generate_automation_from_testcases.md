...
> - **`qa_automation_engineer`** (nạp qua công cụ Skill) — Quy tắc automation chung + workflow routing
> - **`ui_debug_agent`** (nạp qua công cụ Skill) — Inspect DOM, thu thập locators
> - **`smart_locator_agent`** (nạp qua công cụ Skill) — Sinh locator ổn định
> - **`test_data_generator`** (nạp qua công cụ Skill) — Sinh test data unique, traceable
